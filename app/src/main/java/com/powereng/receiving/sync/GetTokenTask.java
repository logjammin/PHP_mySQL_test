...
public class GetTokenTask extends AsyncTask<Void, Void, String> {
	...
	protected String doInBackground(Void... params) {
		...
		if (token != null) {
			...
			ContentResolver.setIsSyncable(account, LogProvider.AUTHORITY, 1);
			ContentResolver.setSyncAutomatically(account,
					LogProvider.AUTHORITY, true);
			// Set sync frequency
			// Set periodic syncing to once every day
            //TODO: might need to remove this if it pertains to actual sync schedule.
			ContentResolver.addPeriodicSync(account, LogProvider.AUTHORITY,
					new Bundle(), 60 * 60 * 24);
